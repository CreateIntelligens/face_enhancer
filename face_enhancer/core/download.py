...
import face_enhancer.core.choices as choices
from face_enhancer.core import curl_builder, logger, state_manager
from face_enhancer.core.filesystem import get_file_name, get_file_size, is_file, remove_file
from face_enhancer.core.hash_helper import validate_hash
from face_enhancer.core.types import Command, DownloadProvider, DownloadSet
...
def conditional_download(download_directory_path : str, urls : List[str]) -> None:
	for url in urls:
		...
		if initial_size < download_size:
			with tqdm(total = download_size, initial = initial_size, desc = 'Downloading', unit = 'B', unit_scale = True, unit_divisor = 1024, ascii = ' =', disable = state_manager.get_item('log_level') in [ 'warn', 'error' ]) as progress:
...
def conditional_download_hashes(hash_set : DownloadSet) -> bool:
	...
	for valid_hash_path in valid_hash_paths:
		...
		logger.debug(f'Validating hash succeeded: {valid_hash_file_name}', __name__)
	for invalid_hash_path in invalid_hash_paths:
		invalid_hash_file_name = get_file_name(invalid_hash_path)
		logger.error(f'Validating hash failed: {invalid_hash_file_name}', __name__)
...
def conditional_download_sources(source_set : DownloadSet) -> bool:
	...
	for valid_source_path in valid_source_paths:
		...
		logger.debug(f'Validating source succeeded: {valid_source_file_name}', __name__)
	for invalid_source_path in invalid_source_paths:
		invalid_source_file_name = get_file_name(invalid_source_path)
		logger.error(f'Validating source failed: {invalid_source_file_name}', __name__)

		if remove_file(invalid_source_path):
			logger.error(f'Deleting corrupt source: {invalid_source_file_name}', __name__)
...
def resolve_download_url_by_provider(download_provider : DownloadProvider, base_name : str, file_name : str) -> Optional[str]:
	download_provider_value = choices.download_provider_set.get(download_provider)
